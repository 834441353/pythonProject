...
class Data:
    def __init__(self):
        ...
        self.N = 60000
        self.M = 10000
        self.BATCHSIZE = 2000  #2000
        self.reg_factor = 0.06   #1e-3
        self.stepsize = 0.015   #1e-2
        ...
        self.init_network()
        #pdb.set_trace()
        self.read_train_images( './MNIST_data/train-images.idx3-ubyte')
        self.read_train_labels( './MNIST_data/train-labels.idx1-ubyte')

        self.train_data = np.append( self.train_img_list, self.train_label_list, axis = 1 )


        self.read_test_images('./MNIST_data/t10k-images.idx3-ubyte')
        self.read_test_labels('./MNIST_data/t10k-labels.idx1-ubyte')
    ...
    def read_train_images(self,filename):
        ...
        for i in range(self.train_img_num):
            ...
            if showimagelable:
                image = np.array(im).reshape((28,28))*255
                #cv2.imshow("teg",image)
                #cv2.waitKey(0)
...
def main():
    ...
    data.save_Data()
    print("OK!")
    #pickle.dump( data.loss_list, open( "gradient_data.txt", "w" ), False )
